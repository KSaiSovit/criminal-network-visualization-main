...
'''
Finds CSV files with specific names within a given directory.
Returns a list of those file names.
'''
...
This function uses Pandas for manipulating DataFrames and to perform data analysis.
Merges multiple CSV files while cleaning and aggregating specific columns based on Jaccard similarity analysis.
'''
...
def merge_removed_edges_results(results_dir_path):
    ...
    merged_df.to_csv(results_dir_path + '/' + FP_DF_OUTPUT)
'''
This function is very similar to merge_jaccard_results, but focuses on merging and aggregating Jaccard similarity results based on the #_removed_edges column:

Similarities:

Initialization: Same as merge_jaccard_results.
Merging files: Same as merge_jaccard_results.
Cleaning up columns: Same as merge_jaccard_results.
Defining aggregation functions: Similar, but only includes #_removed_edges and accuracy columns.
Finalizing and exporting: Same as merge_jaccard_results.

Differences:

Grouping: Groups only by #_removed_edges (not #_central_nodes).
Aggregation: Only aggregates based on the defined functions for each column.

This function performs a simplified analysis focusing solely on the impact of varying numbers of removed edges on the accuracy metrics.
'''
